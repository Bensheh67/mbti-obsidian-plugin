...
function generateTemplate(mbtiType) {
...
class MBTITemplatePlugin extends obsidian_1.Plugin {
    ...
    async createNote(filepath, content) {
        ...
        try {
            ...
            if (existingFile && !this.settings.allowOverwrite) {
                new obsidian_1.Notice(`${filepath} already exists. Enable "Allow Overwrite" in settings to replace.`);
                return;
            }
            ...
            if (parts.length > 1) {
                ...
                try {
                    ...
                    new obsidian_1.Notice(`Failed to create folder: ${dir}`);
                    return;
                }
            }
            ...
            if (existingFile) {
                ...
                if (file && file instanceof obsidian_1.TFile) {
                    ...
                    new obsidian_1.Notice(`Updated: ${filepath}`);
                }
            }
            else {
                await vault.create(filepath, content);
                new obsidian_1.Notice(`Created: ${filepath}`);
            }
        }
        catch (error) {
            console.error('Error creating note:', error);
            new obsidian_1.Notice(`Failed to create note: ${filepath}`);
        }
    }
    onunload() {
...
exports.default = MBTITemplatePlugin;
// Settings Tab
class MBTISettingTab extends obsidian_1.PluginSettingTab {
...
class PreviewModal extends obsidian_1.Modal {
    constructor(app, mbtiType, template) {
        super(app);
